document.addEventListener('DOMContentLoaded', function(){
  ...
  async function fetchApiKeys(){
    const base = getApiBase()
    try{
      const res = await fetch(base + '/bot/api-keys', { headers: headers() })
      if(!res.ok) throw new Error(await res.text())
  ...
  async function createApiKey(){
    const base = getApiBase()
    const name = apiKeyName && apiKeyName.value ? apiKeyName.value.trim() : ''
    const body = name ? { name } : {}
    const res = await fetch(base + '/bot/api-keys', { method:'POST', headers: headers(), body: JSON.stringify(body) })
    if(!res.ok) throw new Error(await res.text())
    return await res.json()
  }

  async function deleteApiKey(token){
    const base = getApiBase()
    const res = await fetch(base + '/bot/api-keys/' + encodeURIComponent(token), { method:'DELETE', headers: headers() })
    if(!res.ok) throw new Error(await res.text())
    return await res.json()
  }
  ...
  async function refreshApiKeys(){
    ...
    apiKeysList.innerHTML = 'Cargando...'
    try{
      const keys = await fetchApiKeys()
      apiKeysList.innerHTML = ''
      if(!keys || keys.length===0){ apiKeysList.textContent = '(sin API keys)'; return }
      keys.forEach(k=>{
        const row = document.createElement('div'); row.style.display='flex'; row.style.alignItems='center'; row.style.justifyContent='space-between'; row.style.marginBottom='6px'; row.style.background='var(--card)'; row.style.padding='6px 8px'; row.style.borderRadius='6px'
        const left = document.createElement('div'); left.style.flex='1';
        left.innerHTML = `<div style="font-weight:700">${k.meta && k.meta.name ? k.meta.name : '(sin nombre)'}</div><div style="color:var(--muted);font-size:0.9rem">${k.key}</div>`
        const btns = document.createElement('div');
        const copyBtn = document.createElement('button'); copyBtn.className='btn ghost'; copyBtn.textContent='Copiar'; copyBtn.style.marginRight='6px'; copyBtn.addEventListener('click', ()=>{ try{ navigator.clipboard.writeText(k.key); copyBtn.textContent='Copiado'; setTimeout(()=>{ copyBtn.textContent='Copiar' }, 1200); }catch(e){} })
        const delBtn = document.createElement('button'); delBtn.className='btn ghost'; delBtn.textContent='Eliminar'; delBtn.addEventListener('click', async ()=>{
          if(!confirm('¿Eliminar esta API key?')) return
          try{ await deleteApiKey(k.key); await refreshApiKeys(); }catch(e){ alert('Error al eliminar: '+(e.message||e)) }
        })
        btns.appendChild(copyBtn); btns.appendChild(delBtn)
        row.appendChild(left); row.appendChild(btns)
        apiKeysList.appendChild(row)
      })
    }catch(e){ apiKeysList.textContent = 'Error: '+(e.message||e) }
  }
  ...
  // API keys: wire create and initial list
  createApiKeyBtn && createApiKeyBtn.addEventListener('click', async ()=>{
    try{
      createApiKeyBtn.disabled = true
      const res = await createApiKey()
      alert('API key creada: ' + (res.key||''))
      if(apiKeyName) apiKeyName.value=''
      await refreshApiKeys()
    }catch(e){ alert('Error creando API key: '+(e.message||e)) } finally { createApiKeyBtn.disabled = false }
  })
  try{ refreshApiKeys() }catch(e){}
