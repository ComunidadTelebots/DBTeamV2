...
if (-Not (Test-Path ".\.venv\Scripts\python.exe")) {
    ...
    if (Test-Path $venvPython) {
        ...
        if (Test-Path "python_api/requirements.txt") {
            & $venvPython -m pip install -r python_api/requirements.txt
        }
    } else {
...
Start-Process -FilePath $venvPython -ArgumentList "-m", "uvicorn", "python_api.app.main:app", "--host", "0.0.0.0", "--port", "8081", "--reload"
