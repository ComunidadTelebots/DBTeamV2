...
(function(){
  ...
  // Login via Bot: request a short code sent by the Telegram bot to the username in the user field
  const btnLoginViaBot = document.getElementById('btnLoginViaBot');
  if(btnLoginViaBot){
    btnLoginViaBot.addEventListener('click', async ()=>{
      const user = (inputUser && inputUser.value) ? inputUser.value.trim() : '';
      if(!user){ show('Introduce @username o username'); return }
      show('Solicitando código al bot...');
      const apiBaseInput = document.getElementById('apiBaseLogin');
      const apiBaseVal = apiBaseInput && apiBaseInput.value ? apiBaseInput.value.replace(/\/+$/, '') : (location.origin || 'http://127.0.0.1:5500');
      try{
        // try same-origin first
        let res = null;
        try{ res = await fetch('/login/request', { method:'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ username: user }) }); }catch(e){}
        if(!res || !res.ok){ try{ res = await fetch(apiBaseVal + '/login/request', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ username: user }) }); }catch(e){ res = null } }
        if(!res){ show('No se pudo contactar al servidor'); return }
        const j = await res.json().catch(()=>null);
        if(!res.ok){ show('Error: '+(j && j.error ? j.error : 'request failed')); return }
        show('Código enviado. Comprueba Telegram y escribe el código que recibiste.');
        const code = prompt('Introduce el código enviado por el bot:');
        if(!code){ show('Código no introducido'); return }
        // verify
        let vres = null;
        try{ vres = await fetch('/login/verify', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ code: code }) }); }catch(e){}
        if(!vres || !vres.ok){ try{ vres = await fetch(apiBaseVal + '/login/verify', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ code: code }) }); }catch(e){ vres = null } }
        if(!vres){ show('Verificación fallida'); return }
        const vd = await vres.json().catch(()=>null);
        if(vd && vd.session){ localStorage.setItem('api_token', vd.session); show('Login OK — redirigiendo...'); setTimeout(()=> location.href='/',600); }
        else{ show('Código inválido o expirado'); }
      }catch(e){ show('Error durante login via bot'); }
    });
  }

  // Handler for Telegram login widget
  window.onTelegramAuth = async function(user){
    show('Verificando Telegram...');
    try{
      const apiBaseInput = document.getElementById('apiBaseLogin');
      const apiBaseVal = apiBaseInput && apiBaseInput.value ? apiBaseInput.value.replace(/\/+$/, '') : (location.origin || 'http://127.0.0.1:5500');
      // try same-origin first
      let res = null;
      try{
        res = await fetch('/auth', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(user) });
      }catch(e){}
      if(!res || !res.ok){
        try{ res = await fetch(apiBaseVal + '/auth', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(user) }); }catch(e){ res = null }
      }
      if(!res || !res.ok){ show('Falló verificación Telegram'); return }
      const data = await res.json();
      if(data && data.token){ localStorage.setItem('api_token', data.token); show('Autenticado vía Telegram. Redirigiendo...'); setTimeout(()=> location.href='/',600); }
      else{ show('Respuesta inválida del servidor'); }
    }catch(e){ show('Error comprobando Telegram'); }
  }
